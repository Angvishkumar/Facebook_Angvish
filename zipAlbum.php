...
//Creates a zip file of photos if user asks to download the album
    ...
    foreach ($photos as $photo) {
        ...
        $filename = $albumName . "_" . $counter++ . ".jpg";
        //
        ...
        // putting the content of url in a filename as stated above
        file_put_contents($filename, file_get_contents($url));
        // adding the images one by one into the zip
        $zip->addFile($filename, $filename . ".jpg");
        array_push($_SESSION['files'], $filename);
        fclose($fp);
        //readfile($url);
    }
    // close the zip created now
    $zip->close();
