    ...
    (function(d) {
     ...
     // jquery to login if the user is already login
        $(document).on(
            ...
            function(){
						$('#connect').html('');
						$('#connect').hide().animate({opacity: 0}, 1000).popover('disable');
        ...
        $("#connect").click(function() {
						//$('#connect').html('');
            displayUserDataAndAlbum();
        });// facebook button click event ends here ..
        ...
        function displayUserDataAndAlbum(){
            FB.login(function(response) {
                if (response.authResponse) {
                    ...
                    $("#connect").html('');// hidding the facebook button ..
										$('#connect').hide().animate({opacity: 0}, 1000).popover('disable');
                    //show all ablums of user ..
                    FB.api('/me/albums', showAlbums);
                
                } else {
								$('#connect').html("<div id=\"clickToConnect\" ><img rel=\"popover\" data-content=\"Click to Login\" data-original-title=\"Login\" id='facebookLoginButton' src=\"img/connect.jpg\" /></div>");
								$('#connect').show().animate({opacity: 1}, 1000).popover('enable');
                }
            }, {
